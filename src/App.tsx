...
import { Message, GameState, Difficulty, NPCPersonality } from './types';
...
export default function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isLoss = finalSuspicion >= 100;
  ...
  if (state.status === 'idle') {
    return (
      <div className="min-h-screen text-slate-100 flex items-center justify-center p-4 font-sans relative overflow-hidden"
        ...
        {/* Green glow orbs */}
        <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-emerald-500/15 rounded-full blur-[180px] pointer-events-none z-0" />
        <div className="absolute bottom-1/3 right-1/4 w-[450px] h-[450px] bg-amber-500/10 rounded-full blur-[160px] pointer-events-none z-0" />
        <div className="absolute top-1/2 left-1/3 w-[400px] h-[400px] bg-violet-500/12 rounded-full blur-[150px] pointer-events-none z-0" />
        <div className="absolute bottom-1/4 left-1/2 w-[350px] h-[350px] bg-blue-500/12 rounded-full blur-[140px] pointer-events-none z-0" />
        {/* Scanlines particles */}
        <div className="absolute inset-0 pointer-events-none z-0">
          {[...Array(30)].map((_, i) => (
            <span key={i} style={{
              ...
              borderRadius: '50%',
              background: Math.random() > 0.6 ? 'rgba(0,255,65,0.4)' : 'rgba(0,200,255,0.25)',
              animation: `homeFloat ${6 + Math.random() * 8}s ease-in-out infinite`,
        ...
        <style>{`
          @keyframes homeFloat { 0%,100% { transform: translateY(0) translateX(0); opacity: 0.2; } 50% { transform: translateY(-30px) translateX(10px); opacity: 0.6; } }
        ...
        >
          ...
          <div className="text-center space-y-4">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.1 }}
              ...
              <div className="absolute -inset-6 bg-blue-500/30 blur-3xl rounded-full animate-pulse" />
              <Terminal className="w-24 h-24 mx-auto text-blue-400 relative" />
            </motion.div>
            <h1 className="text-7xl font-black tracking-tight text-white">FAKE</h1>
            <p className="text-slate-400 text-2xl font-medium">身份不是你的。是这一轮的。</p>
          </div>

          {/* How to play — bold guide */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
            className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl text-left space-y-4"
            style={{ backdropFilter: 'blur(20px)' }}>
            <div className="space-y-3" style={{ fontSize: '19px', lineHeight: 2 }}>
              {[
          ...
          <div className="space-y-3">
            {hasSavedGame && (
              <button onClick={restoreGame}
                className="w-full py-5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold text-xl transition-all shadow-lg shadow-emerald-900/20 flex items-center justify-center gap-2">
                <RefreshCw className="w-5 h-5" />继续上次的潜伏
              </button>
            )}
            <button onClick={() => { clearSavedGame(); setShowThemeSelect(true); }}
              className="w-full py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold text-xl transition-all shadow-lg shadow-blue-900/20 flex items-center justify-center gap-2 group">
              进入论坛
            ...
            {llmTopics ? (
              <div className="grid grid-cols-2 gap-3">
                {llmTopics.map((cat, ci) => (
                  <div key={ci} className="bg-slate-900/60 border border-slate-700/30 rounded-xl p-4 backdrop-blur-sm
                    ...
                    <div className="space-y-1.5">
                      {cat.topics.map((t, ti) => (
                        <button key={ti} onClick={() => { setCustomTheme(t);
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-all ${
                            t === customTheme
                              ...
                              : 'bg-slate-950/50 border-slate-700/20 text-slate-400 hover:border-slate-500/40 hover:text-slate-200'
                          }`}>
                          {t}
                        </button>
                      ))}
              ...
              <div className="text-center py-8">
                <button onClick={async () => {
                  ...
                  className="px-8 py-4 bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 rounded-xl font-bold text-base
                    hover:bg-emerald-600/30 transition-all disabled:opacity-40 font-mono tracking-wider">
                  {isTopicsLoading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="grid gap-3">
              {[
                ...
                <button key={l.id} onClick={() => startGame(l.id as Difficulty, customTheme)}
                  className={`flex items-center gap-5 p-5 bg-slate-900/50 border rounded-2xl transition-all ${l.color} ${l.bg} text-left`}>
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${l.color} ${l.bg}`}>
                    <span className={`font-black text-base ${l.text}`}>{l.label}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between gap-2">
                      <span className={`text-sm font-bold ${l.text}`}>{l.target}存活</span>
          ...
          gameResult === 'defeat' ? (
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0, x: [0, -3, 3, -3, 3, 0] }}
              ...
              <div className="space-y-2">
                <h3 className="text-3xl font-bold text-white">{state.suspicion >= 100 ? '身份已暴露！' : '你放弃了……吗？'}</h3>
              ...
              {gameRecap && (() => {
                ...
                const lines = gameRecap.split('\n').filter(l => l.trim());
                let cur: typeof sections[0] | null = null;
                for (const line of lines) {
                  const m = line.match(/^([\u{1F000}-\u{1FFFF}])\s*(.+)/u) || (() => {
                    const ch = line.charCodeAt(0);
                    if (ch > 127) return [line, line[0], line.slice(1).trim()];
                    return null;
                  })();
                ...
                return (
                  <div className="bg-slate-900/95 border border-slate-600/30 rounded-2xl p-6 text-left relative overflow-hidden shadow-2xl space-y-5"
                    ...
                    <div className="absolute top-4 right-4 text-[10px] font-bold text-rose-500/40 border border-rose-500/30 px-2 py-0.5 rounded rotate-12">CLASSIFIED</div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              {gameRecap && (() => {
                ...
                for (const l of lines) {
                  const m = l.match(/^([\u{1F000}-\u{1FFFF}])\s*(.+)/u) || (() => {
                    const ch = l.charCodeAt(0);
                    if (ch > 127) return [l, l[0], l.slice(1).trim()];
                    return null;
                  })();
                  if (m) { if (cur) sections.push(cur); cur = { emoji: m[1], title: m[2], lines: [] }; }
                ...
                return (
                  <div className="bg-slate-900/95 border border-slate-600/30 rounded-2xl p-6 text-left relative overflow-hidden shadow-2xl space-y-5"
                    ...
                    <div className="absolute top-4 right-4 text-[10px] font-bold text-emerald-500/40 border border-emerald-500/30 px-2 py-0.5 rounded rotate-12">DECLASSIFIED</div>
